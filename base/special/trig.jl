...
Build.RATIONAL && (mulpi_ext(x::Rational) = mulpi_ext(float(x)))
...
Build.COMPLEX && (sinc{T<:Integer}(x::Complex{T}) = sinc(float(x)))
...
Build.COMPLEX && (cosc{T<:Integer}(x::Complex{T}) = cosc(float(x)))
